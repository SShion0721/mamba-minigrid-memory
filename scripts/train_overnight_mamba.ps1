param(
    ...
    [string]$RunName = "slot_memory_mamba3_s11_seed42",
    [int]$Seed = 42,
    ...
    [int]$SlotCount = 4,
    [ValidateSet("query_pool", "iterative")]
    [string]$SlotExtractor = "iterative",
    [int]$SlotIters = 3,
    [double]$SlotMlpRatio = 2.0,
    [ValidateSet("flatten", "fuse")]
    [string]$TemporalTokenMode = "fuse",
    [ValidateSet("none", "episodic_cue")]
    [string]$MemoryKind = "episodic_cue",
    [int]$MemorySlots = 16,
    [int]$MemoryTopK = 4,
    [int]$MemoryWriteWindow = 12,
    [double]$AuxRecallCoef = 0.05,
    [int]$MambaLayers = 2,
    [int]$DState = 64,
    [int]$MambaHeadDim = 64,
    [int]$MambaNGroups = 1,
...
$ArgsList = @(
    ...
    "--slot-extractor", $SlotExtractor,
    "--slot-iters", "$SlotIters",
    "--slot-mlp-ratio", "$SlotMlpRatio",
    "--temporal-token-mode", $TemporalTokenMode,
    "--memory-kind", $MemoryKind,
    "--memory-slots", "$MemorySlots",
    "--memory-topk", "$MemoryTopK",
    "--memory-write-window", "$MemoryWriteWindow",
    "--aux-recall-coef", "$AuxRecallCoef",
    "--spatial-layers", "2",
    "--spatial-heads", "4",
